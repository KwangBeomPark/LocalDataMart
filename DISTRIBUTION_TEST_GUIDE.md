...
## 🚫 1. 배포 사양 및 전제 조건
1. **자가 설치 인스톨러 지원**: 본 프로젝트는 클릭 한 번으로 모든 빌드 구성을 자동 완료하는 `FinanceDataMart_Installer.exe`를 공식 지원합니다.
...
2. 다운로드한 인스톨러를 더블 클릭하여 실행합니다.
   - 프로그램이 실행되면 자동으로 사용자의 **AppData** 로컬 폴더(`C:\Users\<username>\AppData\Local\FinanceDataMart`)에 소스코드 34종을 복사합니다.
